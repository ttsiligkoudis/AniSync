...
namespace AnimeList.Services.Interfaces
{
    ...
    public interface IConfigStore
    {
        ...
        /// <summary>
        /// Stores the user's MediaFusion personal manifest URL. The URL embeds
        /// an encrypted user-data segment generated by MediaFusion's configure
        /// page (binding the user's debrid provider, indexer selection, and
        /// language filters), so we can't construct it ourselves — the user
        /// pastes it in verbatim. Pass null/empty to clear. No-op on unknown UID.
        /// </summary>
        Task SetMediaFusionManifestUrlAsync(string uid, string manifestUrl);

        /// <summary>
        /// Reads the stored MediaFusion manifest URL, or null when none is set
        /// / the UID is unknown. Consumed by the episode-streams endpoint to
        /// decide whether to query MediaFusion alongside Torrentio.
        /// </summary>
        Task<string> GetMediaFusionManifestUrlAsync(string uid);
