...
- [x] **M4 — Library + Discover** (dedicated search-results page still to come): Discover is media-type aware — Anime tabs (Trending/Popular/Seasonal/Airing) + genre filter over `/api/v1/discover/{kind}`, and Movies/Series tabs (Trending/Popular/Anticipated) over `/api/v1/discover/video/{type}/{mode}` (Trakt-backed, added to `ApiController`). Skip-offset "Load more". Library with status tabs + search over `/api/v1/me/list?status=`.
...
- [x] **Notifications** (`/notifications` + header bell): list + unread badge + mark-all-read over `/api/v1/notifications`. `NotificationsController.ResolveCurrentAsync` now also accepts the `X-AniSync-Config` header (resolving uid via `Utils.ResolveConfigAsync`), so it works on both the Web head (session) and the MAUI head (config credential).
- [x] **PWA (Web head)**: `manifest.webmanifest` + icons + apple-touch-icon, and a **conservative network-first service worker** (`service-worker.js`) that caches only the offline fallback + icons and serves the offline page for navigations when offline — it deliberately does *not* precache Blazor's hashed framework assets, so deploys are picked up immediately and nothing goes stale. Registered from `App.razor`. Remaining: web `<track>` subtitles, push.
