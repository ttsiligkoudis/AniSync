...
- [x] **M4 — Library + Discover** (dedicated search-results page still to come): Discover is media-type aware — Anime tabs (Trending/Popular/Seasonal/Airing) + genre filter over `/api/v1/discover/{kind}`, and Movies/Series tabs (Trending/Popular/Anticipated) over `/api/v1/discover/video/{type}/{mode}` (client wired; **the server endpoint is pending — see the follow-up prompt — until then video mode shows empty**). Skip-offset "Load more". Library with status tabs + search over `/api/v1/me/list?status=`.
