...
- [x] **Notifications** (`/notifications` + header bell): list + unread badge + mark-all-read over `/api/v1/notifications`. **Caveat:** that controller is *session*-authenticated server-side, so it works on the Web head as-is; the MAUI head needs `NotificationsController` to also accept the `X-AniSync-Config` header (small server change) to populate. Remaining: PWA/offline parity on the Web head, subtitle UI, server scrobble, push.
